...
public class SplineVehicleMovementSystem : SystemBase
{
    protected override void OnUpdate()
    {
        ...
        var getTranslationComponentData = GetComponentDataFromEntity<Translation>();
        ...
        Entities.ForEach((ref Translation translation, ref CarComponentData carComponentData, ref Rotation rotation, in Entity carEntity) =>
        {
            if (!carComponentData.IsBus)
            {
                ...
                if (carComponentData.HasReachedDestination)
                {
                    ...
                    entityManager.SetComponentData(carComponentData.splineStart, mySplineStartComponentData);
    ...
    static int GetIndex(int splineIndex, int splineBufferLength, TrackComponentData trackComponentData)
    {
        if (trackComponentData.IsOnStreet && !trackComponentData.IsForward)
        {
            return splineBufferLength - splineIndex - 1;
        }
        else
        {
            return splineIndex;
