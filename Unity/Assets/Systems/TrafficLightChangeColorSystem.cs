        ...
        if (((elapsedTime / trafficLightTimeSwitch) % 1) < 0.1 || ((elapsedTime / trafficLightTimeSwitch) % 1) > 0.9)
            Entities.ForEach((ref TrafficLightComponentData trafficLightComponentData, in Entity trafficLight) =>
            {
                ...
                if (getTrafficLightCrossComponentDataFromEntity.HasComponent(trafficLightCross.Value))
                {
                    ...
                    {
                        var spline1ComponentData = getSplineComponentDataFromEntity[trafficLightComponentData.Spline1];
                        spline1ComponentData.isOccupied = !trafficLightComponentData.isGreen;
                        entityManager.SetComponentData(trafficLightComponentData.Spline1, spline1ComponentData);
                    }

                    if (trafficLightComponentData.Spline2 != Entity.Null)
                    {
                        var spline2ComponentData = getSplineComponentDataFromEntity[trafficLightComponentData.Spline2];
                        spline2ComponentData.isOccupied = !trafficLightComponentData.isGreen;
                        entityManager.SetComponentData(trafficLightComponentData.Spline2, spline2ComponentData);
                    }
                }
            }).Run();
